// Code generated by ogen, DO NOT EDIT.
package api

type V1AdminAuthRefreshPostRes interface {
	v1AdminAuthRefreshPostRes()
}

type V1AdminAuthSigninPostRes interface {
	v1AdminAuthSigninPostRes()
}

type V1AdminAuthUserGetRes interface {
	v1AdminAuthUserGetRes()
}

type V1AdminUsersGetRes interface {
	v1AdminUsersGetRes()
}

type V1AdminUsersPostRes interface {
	v1AdminUsersPostRes()
}

type V1AdminUsersUserIDGetRes interface {
	v1AdminUsersUserIDGetRes()
}

type V1AuthRefreshPostRes interface {
	v1AuthRefreshPostRes()
}

type V1AuthSigninPostRes interface {
	v1AuthSigninPostRes()
}

type V1AuthSignupPostRes interface {
	v1AuthSignupPostRes()
}

type V1AuthUserGetRes interface {
	v1AuthUserGetRes()
}

type V1CatalogCitiesPostRes interface {
	v1CatalogCitiesPostRes()
}

type V1CatalogObjectsGetRes interface {
	v1CatalogObjectsGetRes()
}

type V1CatalogObjectsPostRes interface {
	v1CatalogObjectsPostRes()
}

type V1CatalogRegionsPostRes interface {
	v1CatalogRegionsPostRes()
}

type V1CatalogServicesGetRes interface {
	v1CatalogServicesGetRes()
}

type V1CatalogServicesPostRes interface {
	v1CatalogServicesPostRes()
}

type V1OrganizationsOrganizationIDTendersGetRes interface {
	v1OrganizationsOrganizationIDTendersGetRes()
}

type V1SurveyPostRes interface {
	v1SurveyPostRes()
}

type V1TendersGetRes interface {
	v1TendersGetRes()
}

type V1TendersPostRes interface {
	v1TendersPostRes()
}

type V1TendersTenderIDGetRes interface {
	v1TendersTenderIDGetRes()
}

type V1TendersTenderIDPutRes interface {
	v1TendersTenderIDPutRes()
}

type V1UsersUserIDGetRes interface {
	v1UsersUserIDGetRes()
}
