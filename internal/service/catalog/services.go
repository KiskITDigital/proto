...
func (s *Service) CreateService(ctx context.Context, params CreateServiceParams) (models.CatalogService, error) {
	service, err := s.catalogStore.CreateService(ctx, s.psql.DB(), store.CatalogCreateServiceParams{
		Name:     params.Name,
		ParentID: params.ParentID,
	})
	if err != nil {
		return models.CatalogService{}, fmt.Errorf("create service: %w", err)
	}
